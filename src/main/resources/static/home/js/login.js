...
$(document).ready(function() {
	...
	$('#login-sn').focus(function(){
		$('#loginCue').html("<font ><b>请输入用户ID</b></font>");
	}).blur(function(){
		if($('#login-sn').val().length == 0){
			$('#loginCue').html("<font color='red'><b>ID不能为空</b></font>");
			return false;
		}else{
			$.ajax({
				type: "POST" ,
				url: "check_sn",
				data: "pn=" + $("#login-sn").val() + '&flag=' + 1,
				dataType:'json',
				success: function(res) {
					...
					if (res.data == true) {
						...
						$("#loginCue").html("<font color='red'><b>ID不存在</b></font>");
...
//注册
		...
		$.ajax({
			...
			success: function(res) {
				...
				if(res.data == false){
					...
					$("#userCue").html("<font color='red'><b>ID已存在</b></font>");
					return false;
				}else {
					$('#user-sn').css({
						border: "1px solid #D7D7D7",
	...
	$('#reg_button').click(function() {
		...
		if ($('#user-sn').val().length!=8 || regusersn.test($('#user-sn').val()) == false) {
			...
			$('#userCue').html("<font color='red'><b>请输入8位数字ID</b></font>");
			return false;
		}else {
			$('#user-sn').css({
				border: "1px solid #D7D7D7",
