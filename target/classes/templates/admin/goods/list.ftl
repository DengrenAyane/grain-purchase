            ...
            <div class="card">
                  ...
                  <div class="input-group">
                    <div class="input-group-btn">
                      <button class="btn btn-default dropdown-toggle" id="search-btn" data-toggle="dropdown" type="button" aria-haspopup="true" aria-expanded="false">
                      	<#if goodsCategoryName??>分类名称<#elseif pn??>用户ID<#elseif name??>粮食名称<#else>搜索条件</#if> <span class="caret"></span>
                      </button>
                      <ul class="dropdown-menu">
                        <li> <a tabindex="-1" href="javascript:void(0)" data-field="name">粮食名称</a> </li>
                        <li> <a tabindex="-1" href="javascript:void(0)" data-field="peasant.pn">用户ID</a> </li>
                        <li> <a tabindex="-1" href="javascript:void(0)" data-field="goodsCategory.name">分类名称</a> </li>
                      </ul>
                    </div>
                    <input type="text" class="form-control" value="${name!pn!goodsCategoryName!""}" id="search-value" name="<#if goodsCategoryName??>goodsCategory.name<#elseif pn??>peasant.pn<#else>name</#if>" placeholder="请输入搜索值">
              ...
              <div class="card-body">
                ...
                <ul class="pagination ">
                  ...
                  <#else>
                  <li><a href="list?<#if goodsCategoryName??>goodsCategory.name<#elseif pn??>peasant.pn<#else>name</#if>=${name!pn!goodsCategoryName!""}&currentPage=1">«</a></li>
                  </#if>
                  <#list pageBean.currentShowPage as showPage>
                  <#if pageBean.currentPage == showPage>
                  <li class="active"><span>${showPage}</span></li>
                  <#else>
                  <li><a href="list?<#if goodsCategoryName??>goodsCategory.name<#elseif pn??>peasant.pn<#else>name</#if>=${name!pn!goodsCategoryName!""}&currentPage=${showPage}">${showPage}</a></li>
                  </#if>
                  </#list>
                  <#if pageBean.currentPage == pageBean.totalPage>
                  <li class="disabled"><span>»</span></li>
                  <#else>
                  <li><a href="list?<#if goodsCategoryName??>goodsCategory.name<#elseif pn??>peasant.pn<#else>name</#if>=${name!pn!goodsCategoryName!""}&currentPage=${pageBean.totalPage}">»</a></li>
                  </#if>
